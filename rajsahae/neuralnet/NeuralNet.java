 ...
 * layer, and a specified number of hidden layers.  Feed it inputs of type <T> and it will
 * output values of type <T> as long as <T> is compatible with mathematical operations
 ...
 * @version 1.0
 *
 * @param <T> Variable Type of input.  Should be compatible with mathematical operations
 * 
 */
public class NeuralNet {
	...
	public List<Double> update(ArrayList<Double> inputs) {
		ArrayList<Double> outputs = new ArrayList<Double>();
		
		// check for correct number of inputs
		assert(inputs.size() == this.numInputs);
		...
		for (int i=0; i<this.neuronLayers.size(); i++) {
			...
			/*
			System.out.println("Neuron Layer " + (i+1)
					+ " num Neurons is: " + this.neuronLayers.get(i).getNumNeurons());
			*/
			for (int j=0; j<this.neuronLayers.get(i).getNumNeurons(); j++) {
				...
				/*
				System.out.println("Num inputs to Neuron " + (j+1) + " is: "
						+ numInputs);
				System.out.println(this.neuronLayers.get(i).neurons.get(j).toString());
				System.out.println("Output for this neuron is: "
						+ sigmoid(netInput, this.neuronLayers.get(i).neurons.get(j)));
				*/
				// Enter the net input into the sigmoid function and store the output
				outputs.add(sigmoid(netInput, 
									this.neuronLayers.get(i).neurons.get(j) ));
			}
		}
	...
	protected static class Neuron {
		...
		protected Neuron(int numInputs, double activationThreshold){
			...
			Neuron.randomizeNeuronWeights(this, numInputs, 0);
		}
		...
		protected static void randomizeNeuronWeights(Neuron neuron, int weights,
			...
			for (int i=0; i<weights; i++) {
				neuron.inputWeights.add(randGen.nextDouble());
			}
